...
// ── SVG icon helpers (React.createElement — no JSX needed in .ts) ────────────
...
export const GmailIcon = svgIcon(
...
export const TelegramIcon = svgIcon(
...
export const SHARE_OPTIONS: ShareOption[] = [
  {
    ...
    color: "#25D366",
    getUrl: (url) =>
      `https://wa.me/?text=${encodeURIComponent("Join my secure chat room: " + url)}`,
  },
  {
    label: "Gmail",
    icon: React.createElement(GmailIcon),
    color: "#EA4335",
    getUrl: (url) =>
      `mailto:?subject=${encodeURIComponent("Join my chat room")}&body=${encodeURIComponent("Tap the link to join: " + url)}`,
  },
  {
    label: "Telegram",
    ...
    color: "#229ED9",
    getUrl: (url) =>
      `https://t.me/share/url?url=${encodeURIComponent(url)}&text=${encodeURIComponent("Join my secure chat room")}`,
  },
  {
    label: "Twitter / X",
    ...
    color: "#1DA1F2",
    getUrl: (url) =>
      `https://twitter.com/intent/tweet?text=${encodeURIComponent("Join my secure chat room: " + url)}`,
  },
];
